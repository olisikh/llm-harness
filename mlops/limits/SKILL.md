---
name: limits
description: Check remaining LLM quota windows for Hermes-configured primary and fallback providers with codexbar, formatted concisely for humans.
---
...
- Default to Hermes `model.provider` plus `fallback_providers`; skip auxiliary providers unless explicitly requested.
- Keep user-facing output concise.

## Options

```bash
python ~/.hermes/skills/mlops/limits/scripts/limits.py --include-aux
